...
Motor::Motor(uint8_t stepPin, uint8_t dirPin, float stepsPerUnit, int eepromAddr)
...
{
    return currentPosition() / _stepsPerUnit;
}

float Motor::getTargetPos()
{
    return _targetPos / _stepsPerUnit;
}

float Motor::getMaxSpeedUnit()
{
    return maxSpeed() / _stepsPerUnit;
}

float Motor::getAccelerationUnit()
{
    return acceleration() / _stepsPerUnit;
}

void Motor::setTargetPos(float pos)
{
    float newTarget = pos * _stepsPerUnit;
    if (_targetPos == newTarget)
        return;
...
void Motor::handleControlWord(uint16_t cw)
{
    ...
    if (currentTrigger && !_lastTriggerState) {
        if (!_running) {
            ...
            _lastPos = _targetPos;
            _runed = true; // 运动触发时立即将 _runed 置为 true
        }
    }
...
uint16_t Motor::getStatusWord()
{
    ...
    if (_running) {
        ...
        if (_runed && distanceToGo() == 0) {
            ...
            _runed = false;
            savePositionToEEPROM(); // 运动完成到达目标位置时，自动持久化当前位置到
                                    // EEPROM
        }
    }
...
void Motor::setError(bool error)
{
    _errorFlag = error;
}
